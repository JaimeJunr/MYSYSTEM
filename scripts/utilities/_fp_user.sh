#!/usr/bin/env bash
# Template: um link simbólico por app; o nome do arquivo (sem .sh) é o ID Flatpak no Flathub.
set -euo pipefail
_hs_find_root() {
  local d="$1"
  while [[ "$d" != "/" ]]; do
...
hs_os_release
hs_info "Instalando via Flatpak (usuário): ${REF}"
hs_flatpak_install_user "${REF}"
